-- venues_nearby: return good_for and accept p_features so the recommender can
-- pull venues with a requested feature (kabinet, karaoke…) city-wide.
drop function if exists public.venues_nearby(double precision, double precision, integer, text, text[], integer, text[], text[], text[], text);
create or replace function public.venues_nearby(
  ...
  p_tier text default null, p_features text[] default null
) returns table (
  ...
  select v.id, v.slug, v.name, v.category, v.cuisines, v.lat, v.lon,
   ...
   where v.is_active
     ...
     and (p_features is null or v.features && p_features)
