...
- **2026-09-15** — Ali: "xəngəl restoranı" yanlış sonuç veriyor ve uzun sürüyor. **Kök neden:** DeepSeek V4'e geçti; `deepseek-chat` artık düşünen (reasoning) modele bağlanıyor, 600 token bütçesini düşünmeye harcayıp boş cevap dönüyor ya da 10+ sn sürüyordu; zaman aşımında kural ayrıştırıcısı devreye giriyor, o da "xəngəl"i bilmiyordu. **Çözüm:** DeepSeek isteklerinde `thinking: disabled` (1 sn'ye düştü), sağlayıcı zinciri (DeepSeek → Groq → Gemini) + 90 sn soğuma, niyet önbelleği, kural ayrıştırıcısına 36 yemeklik sözlük (xəngəl = khinkali), Azerice ə normalizasyonu, yemek adı yorumlarda tüm yazımlarıyla aranıyor. Test seti 25/25.
